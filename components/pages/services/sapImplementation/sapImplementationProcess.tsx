...
const SapImplementationProcess: React.FC<SapImplementationProcessProps> = ({
  ...
  return (
    <div className="flex lg:h-[75vh] relative items-center justify-start flex-col gap-10 bg-gray-200 py-20 lg:mb-56">
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:absolute top-1/2`}>
        {SapImplemetationProcessData.steps.map((item, index) => (
          <div key={index} className="flex items-center justify-center flex-col gap-10 group">
            <div className="rounded-full border-[#b00d07] group-hover:bg-gray-200 border-dashed border p-10">
              <Image className="group-hover:scale-125 transition duration-500" alt="" src={item.imagePath} height={100} width={100} />
            </div>
