...
const AnimatedCard = ({ item, trigger }: AnimatedCardProps) => {
  return (
    <Card className="w-full h-full absolute top-10 lg:top-1/3 border-none rounded-none flex flex-col items-center md:items-start gap-10 lg:gap-3 sm:gap-10 py-5 3xl:gap-20 bg-transparent backdrop-blur-sm px-4 md:pl-32">
      ...
      >
        <CardTitle className="text-2xl md:pl-6 lg:text-4xl 2xl:text-6xl 3xl:text-7xl font-semibold z-20 text-black">
          {item.title} <br />
      ...
      <CardContent className="flex flex-col items-center md:items-start text-center md:text-left gap-10 3xl:gap-20 w-full">
        <motion.span
          className="text-sm lg:text-md 2xl:text-2xl 3xl:text-2xl text-black w-full md:w-1/2"
          initial={{ opacity: 0, x: 100 }}
          animate={trigger ? { opacity: 1, x: 0 } : { opacity: 0, x: 100 }}
